...
Bash is like regular expressions: everyone uses it, but nobody knows it well. Every time I need to write Bash, I find
...
language, using a few lines of Bash could lead to a much simpler code. For me, Bash is useful for
...
`/bin/sh` is just a symbolic link to `Bash`, `Dash`, etc. To check what is your default shell use `echo "$SHELL"`.
...
and return the value. In some cases it may be useful to fail with an error message if the variable is not set `${variable?message}`.
...
For iterating until the brake condition is met, use `while` loop. The popular use case is [iterating over lines of a file](https://stackoverflow.com/a/10929511/3986320).
...
Remember to use the semicolon `;` when writing multiple commands in a single line, this also applies to `if ...; then`,
...
status](https://en.wikipedia.org/wiki/Exit_status). To provide an exit code using `exit 0` for success,
...
for example, `/usr/bin/`, you can call it by just invoking its filename. An example of a trivial script is given below.
...
redirecting to a file, `>` will overwrite the target file, to append it use `>>` instead. Use `2>` if you want to
...
To display a list of active jobs use the `jobs` command.  `fg [job_spec]` moves the job to the foreground, Ctrl+Z or
`bg [job_spec]` to the background, `disown [job_spec]` terminates it. To prevent the processes from dying with the shell
...
the built-in commands use `help` alone.
...
It conducts a static analysis of the script and provides many helpful hints for solving the issues.

If you want to add unit tests to your code, there is a [useful `assert.sh` script](https://torokmark.github.io/assert.sh/)
and the great [bats](https://github.com/bats-core/bats-core) testing framework.
