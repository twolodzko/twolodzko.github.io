...
I searched all the internet and saw the re-occurring advice: *"don't"*. They gave all the good arguments, but I *needed*
...
Here I was positively surprised by the [`enum`] type that fitted great for the purpose. It supports all the possible kinds
...
For my variables, I didn't only want to store them. I also needed to do things like copying, printing or comparing them.
Does this mean that I needed to implement all those things? No, Rust is smart enough to do it, I only needed to
add `#[derive()]`'s to it. For example, to be able to compare if two lists are the same, I just needed to derive the
`PartialEq` trait, to be able to copy them, the `Clone` trait and `Debug` trait was needed for pretty printing for
debugging and unit testing. If all the elements of the list follow those traits, and the list follows the traits,
it is enough for it to work without any additional code.
...
There are also traits that I implemented myself, like `Display` to print the values, `Iter` to iterate over the lists,
`From<T>` to convert from the type `T` into the type that implemented it, etc.

In fact, `#[derive()]` has saved my implementation. Initially, I implemented `List::clone` by myself in a very inefficient
...
they are tree-like structures (multiple environments can have the same parent), and each environment is a mapping
([`HashMap`]) and we need read and write access. `Rc`, which I used in lists, does not allow for writing, so additionally
I used [`RefCell`] that allows for mutable [borrowing] of the values. There are several other similar constructs
(e.g. [`Mutex`]), but I needed my interpreter to run single-threaded, so the simple `RefCell` sounded good enough and
would likely be more efficient. The final implementation was defined in terms of a recursive data structure:
...
It is a nice example of how Rust encourages to use of its standard library provides a lot of useful
...
using generics a few times helped me to improve and simplify the code.
...
match sexpr {
    Sexpr::Symbol(ref name) => {
        return env.get(name).ok_or(Error::NotFound(name.clone()))
    },
...
operations provided in the standard library like `.ok_or(err)` that translate the `Option::Some` result to `Resul::Ok` and
`Option::None` to `Result::Err` for the given `err` error message. That's a nice use of monads. For `Quote` we
needed to unbox the `quoted` value by `*quoted`. In several places, I also needed to `.clone()` the values (recall, Rust
does not allow us to re-use them).

It seemed to work, but only till I learned that Rust [is not tail-call optimized]. The recursion that is used everywhere
...
implement by providing the `.next()` method and it would give us for free a number of other methods like `map`, `filter`,
`collect`, etc. But it was not only about "nicer" code, but rather simplified the code a lot by giving me access to many
utilities from the standard library (told you so).
...
my iterator that stops on an error and saves the error message, otherwise returning the `Option<Sexpr>` results. It
defines the `.next()` method from the `Iter` trait as below.
...
Returning `None` signalizes that it was the final iteration. If you wonder what is the small `?` in the second line,
...
It helped me to write procedures like [`list`] using iterators, with just a few lines of code.
The example below shows an iterator that would go through all the `args` while evaluating them, then
...
All the unit tests passed. It was finally time for an integration test. This meant that now I needed not only to evaluate
...
implemented for reading from strings. The file could be processed like this line-by-line, for REPL it'd mean prompting
the user for new lines. Easy peasy, I thought. But no, it wasn't. If I read a line it is saved to a temporary variable,
Rust does not allow to preserve a state that would depend on such a temporary variable, because it'd be dropped, so
it won't be available anymore. I was struggling a bit, trying to add [lifetimes], etc but with no luck. [The solution]
