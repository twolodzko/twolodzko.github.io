---
title: "Imitating Rusts error propagation in Haskell"
date: 2023-02-01
categories: [blog]
---
...
Rust uses the [`Result`] [monad]. The type can take two possible values, `Ok` and `Err`, for the result of the happy
...
Haskell would not allow for such an early return. But let's not jump ahead. First of all, Haskell leaves to the user
how to handle the errors, without suggesting solutions. As Rust, it has the `Maybe` type for nullable values, but it
does not have `Result`. Instead, it has the [`Either` type] with `Left` and `Right` fields.
Don't know like you, but I feel that sooner or later I'll forget which value was "Left" and which was "Right".
Hopefully, nothing prohibits us from creating our type like below.
